...
$(document).ready(function () {
        ...
        vmFormBMI.Guardar = function () {
            ...
            if (vmFormBMI.DatosExcelIDS().length > 0 && vmFormBMI.NombreDocumento() != "") {
                $.ajax({
                    ...
                    url: "/DatosExcelInstituciones/Generar",
                    data: JSON.stringify({
                        Ids: ko.toJS(vmFormBMI.DatosExcelIDS())
                    }),
                    success: function (Data) {
                        if (Data != null) {
                            window.open(`/DatosExcelInstituciones/ExportarExcel?cabecera=${Data.cabecera}&NombreDocumento=${vmFormBMI.NombreDocumento()}`, "_blank");
                        }
                        vmFormBMI = {};
    ...
    $.ajax({
        type: "GET",
        contentType: "application/json; charset=utf-8",
        url: "/DatosExcelInstituciones/GetFormulario?id=" + vmh.CurrentId(),
        success: KnockoutFormBMI
    });
});

function ExportarExcel() {
    window.location = "/DatosExcelInstituciones/ExportarExcel?Datos="; //+ vmFormBMI.DatosExcelIDS();
}